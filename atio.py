...
def main():
    clear()
    setTitle(f"@Dark V1 Menu v{THIS_VERSION}")
    astraahometitle()
...
\n                                      {y}[{w}23{y}]{w} Remover                 {y}[{w}25{y}]{w} Exit\n\n\n\n\n\n\n\n\n\n                                                                                                     {y}[{b}<{y}]{w} Previous Page""")
        ...
        if choice == '21':
            ...
            transition()
            astraahometitle()
            print(f"""                                            {y}[{b}+{y}]{w} Development Networks:\n\n                                                {y}[{w}#{y}]{w} Yt: @TermedModz\n                                                {y}[{w}#{y}]{w} Server:    None\n\n\n                                            {y}[{b}+{y}]{w} Other Network\n\n                                                {y}[{w}#{y}]{w} Twitter:   @siresirol837\n                                                {y}[{w}#{y}]{w} Discord:   officialvipmodz\n\n\n\n\n""")
...
if __name__ == "__main__":
    ...
    try:
        ...
        input(f"{y}[{Fore.RED}#{y}]{w} Sorry but, your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with @Dark V1, please download python 3.9 or higher.")
        sys.exit()
    else:
        #search_for_updates()
        clear()
